...
def load_everything(args, cfg):
    ...
    if cfg.data.dataset_type == "waymo":
        ...
        data_dict = load_common_data(cfg.data, cfg)
    
    # remove useless field
    kept_keys = {'hwf', 'HW', 'Ks', 'near', 'far', 'near_clip',
            'i_train', 'i_val', 'i_test', 'irregular_shape',
            'poses', 'render_poses', 'images', 'edgeimages'}
    ...
    if data_dict['irregular_shape']:
        ...
        data_dict['edgeimages'] = [torch.FloatTensor(im, device='cpu') for im in data_dict['edgeimages']]
    else:
        data_dict['images'] = torch.FloatTensor(data_dict['images'], device='cpu')
        data_dict['edgeimages'] = torch.FloatTensor(data_dict['edgeimages'], device='cpu')
