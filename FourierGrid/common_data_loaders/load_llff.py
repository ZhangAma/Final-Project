...
def _load_data(basedir, edgeType, factor=None, width=None, height=None, load_imgs=True, load_depths=False):
    ...
    edgeimags = np.stack(edgeimags, -1) # (N, H, W)

    print('Loaded image data', imgs.shape, poses[:,-1,0])
    print('Loaded edge data', edgeimags.shape, poses[:,-1,0])
    
    if not load_depths:
        return poses, bds, imgs, edgeimags
    ...
    return poses, bds, imgs, edgeimags, depths
...
def load_llff_data(basedir, edgeType, factor=8, width=None, height=None,
                   recenter=True, rerotate=True,
                   bd_factor=.75, spherify=False, path_zflat=False, load_depths=False,
                   movie_render_kwargs={}):

    poses, bds, imgs, edgeimgs, *depths = _load_data(basedir, edgeType, factor=factor, width=width, height=height,
    ...
    images = imgs
    edgeimgs = np.moveaxis(edgeimgs, -1, 0).astype(np.float32)
    edgeimages = edgeimgs
...
#             zloc = np.percentile(tt, 10, 0)[2]
    ...
    edgeimages = edgeimages.astype(np.float32)

    return images, edgeimages, depths, poses, bds, render_poses, i_test
