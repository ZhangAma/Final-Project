...
def create_new_model(data_dict, args, cfg, cfg_model, cfg_train, xyz_min, xyz_max, stage, coarse_ckpt_path, device):
    ...
    if cfg.data.dataset_type == "waymo" or cfg.data.dataset_type == "nerfpp" or cfg.data.dataset_type == "tankstemple" \
        ...
        model = FourierGridModel(
            ...
            num_voxels_density=num_voxels_density, num_voxels_rgb=num_voxels_rgb, verbose=verbose, edgenet=cfg.edgeLoss, edgetraintype=cfg.edgeTrainType,
            **model_kwargs)
    elif cfg.data.ndc:
...
def gather_training_rays(data_dict, images, cfg, i_train, cfg_train, poses, HW, Ks, model, render_kwargs):
    ...
    if cfg.data.dataset_type in FourierGrid_datasets or cfg.model == 'FourierGrid':
        rgb_train, rays_original_point_train, rays_direction_train, viewdirs_train, indexs_trainain, imsz = model.FourierGrid_get_training_rays(
        rgb_train_ori=rgb_train_ori, train_poses=poses[i_train], HW=HW[i_train], Ks=Ks[i_train], 
    ...
    return rgb_train, rays_original_point_train, rays_direction_train, viewdirs_train, indexs_trainain, imsz, batch_index_sampler
...
def scene_rep_reconstruction(args, cfg, cfg_model, cfg_train, xyz_min, xyz_max, data_dict, stage, coarse_ckpt_path=None):
    ...
    HW, Ks, near, far, i_train, i_val, i_test, poses, images, edgeimages = [
    ...
    FourierGrid_datasets = ["waymo", "mega", "nerfpp", "tankstemple"]
    if cfg.data.dataset_type in FourierGrid_datasets or cfg.model == 'FourierGrid':
        rgb_train, edge_train, rays_original_point_train, rays_direction_train, viewdirs_train, indexs_train, imsz, batch_index_sampler = \
            model.gather_training_rays(data_dict, [images, edgeimages], cfg, i_train, cfg_train, poses, HW, Ks, render_kwargs)
    else:
        rgb_train, rays_original_point_train, rays_direction_train, viewdirs_train, indexs_train, imsz, batch_index_sampler = gather_training_rays(
            data_dict, [images, edgeimages], cfg, i_train, cfg_train, poses, HW, Ks, model, render_kwargs
        )
    # view-count-based learning rate
    if cfg_train.pervoxel_lr:
        def per_voxel_init():
            cnt = model.voxel_count_views(
                    rays_original_point_train=rays_original_point_train, rays_direction_train=rays_direction_train, imsz=imsz, near=near, far=far,
    ...
    if cfg_train.maskout_lt_nviews > 0:
        model.update_occupancy_cache_lt_nviews(
                rays_original_point_train, rays_direction_train, imsz, render_kwargs, cfg_train.maskout_lt_nviews)
    ...
    for global_step in trange(1 + start, 1 + training_steps):
        ...
        if cfg_train.ray_sampler in ['flatten', 'in_maskcache']:
            ...
            target = rgb_train[sel_i]
            target_edge = edge_train[sel_i]
            rays_o = rays_original_point_train[sel_i]
            rays_d = rays_direction_train[sel_i]
            viewdirs = viewdirs_train[sel_i]
            if indexs_train is not None:
                indexs = indexs_train[sel_i]
            else:
                indexs = None
        elif cfg_train.ray_sampler == 'random':  # fixed function
            if len(rgb_train.shape) != 2:
                sel_b = torch.randint(rgb_train.shape[0], [cfg_train.N_rand])
                sel_r = torch.randint(rgb_train.shape[1], [cfg_train.N_rand])
                sel_c = torch.randint(rgb_train.shape[2], [cfg_train.N_rand])
                target = rgb_train[sel_b, sel_r, sel_c]
                rays_o = rays_original_point_train[sel_b, sel_r, sel_c]
                rays_d = rays_direction_train[sel_b, sel_r, sel_c]
                viewdirs = viewdirs_train[sel_b, sel_r, sel_c]
                if indexs_train is not None:
                    indexs = indexs_train[sel_b, sel_r, sel_c]
                else:
                    indexs = None
            else:
                assert len(rgb_train.shape) == 2, "tgb_tr's shape is not correct."
                sel_b = torch.randint(rgb_train.shape[0], [cfg_train.N_rand])
                sel_r = torch.randint(rgb_train.shape[1], [cfg_train.N_rand])
                target = rgb_train[sel_b]
                rays_o = rays_original_point_train[sel_b]
                rays_d = rays_direction_train[sel_b]
                viewdirs = viewdirs_train[sel_b]
                if indexs_train is not None:
                    indexs = indexs_train[sel_b]
                else:
                    indexs = None
        else:
            raise NotImplementedError
        ...
        render_result = model(rays_o, rays_d, viewdirs, global_step=global_step, is_train=True,
            **render_kwargs) # 'near', 'far', 'bg', 'rand_bkgd', 'stepsize', 'inverse_y', 'flip_x', 'flip_y', 'indexs'
        ...
        if cfg.edgeLoss and cfg.edgeTrainType == "FitEdge":
            mse_loss = F.mse_loss(render_result['rgb_marched'], target)
            edge_mse_loss = F.mse_loss(render_result['edge_marched'], target_edge)
            freq_loss = fourier_mse_loss(render_result['rgb_marched'], target)
            psnr = utils.mse2psnr(mse_loss.detach())
            loss = cfg_train.weight_main * mse_loss + cfg_train.weight_freq * freq_loss + cfg_train.weight_edge * edge_mse_loss
        elif cfg.edgeLoss and cfg.edgeTrainType == "EdgeWeight":
            def weighted_mse_loss(input, target, weight):
                ...
                # 用权重乘以逐元素的平方差
                weighted_squared_errors = weight * squared_errors
                # 计算平均值作为损失
                loss = torch.mean(weighted_squared_errors)
                return loss
            weights = target_edge.unsqueeze(1).repeat(1, 3)
            mse_loss = weighted_mse_loss(render_result['rgb_marched'], target, (weights + 1) / 2) # F.mse_loss(render_result['rgb_marched'], target)
            freq_loss = fourier_mse_loss(render_result['rgb_marched'], target)
