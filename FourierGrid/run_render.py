...
def render_viewpoints(cfg, model, render_poses, HW, Ks, ndc, render_kwargs,
    ...
    for i, c2w in enumerate(tqdm_bar):
        ...
        keys = ['rgb_marched', 'edge_marched', 'depth', 'alphainv_last']
        ...
        if cfg.edgeLoss and cfg.edgeTrainType == 'FitEdge':
            edge = render_result['edge_marched'].cpu().numpy()
        
        def enhance_edges(color_image, binary_edges):
            # print(color_image.shape, binary_edges.shape, "color_image.shape, binary_edges.shape")
            edges_colored = cv2.cvtColor(binary_edges, cv2.COLOR_GRAY2BGR)
            alpha = cfg.edgeRenderWeight # 设置透明度，范围为0到1，可以根据需要调整
            enhanced_color_image = cv2.addWeighted(color_image, 1 - alpha, edges_colored, alpha, 0)
            return enhanced_color_image
        
        if cfg.edgeLoss and cfg.edgeTrainType == 'FitEdge':
            rgb = enhance_edges(rgb, edge)
        ...
        if gt_imgs is not None and render_factor==0:
            ...
            # print("rgb image: ", rgb.shape, "gt_imgs[i]: ", gt_imgs[i].shape)
            
            gt_rgb = gt_imgs[i]
            
            # Calculate the absolute difference between the two images
            diff = cv2.absdiff(rgb, gt_rgb)

            # Convert the differences to grayscale
            gray_diff = cv2.cvtColor(diff, cv2.COLOR_BGR2GRAY)

            # Create a heatmap by normalizing the differences to the range [0, 1]
            heatmap = cv2.normalize(gray_diff, None, alpha=0, beta=1, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)

            # Apply a colormap to the heatmap
            colored_heatmap = cv2.applyColorMap(np.uint8(heatmap * 255), cv2.COLORMAP_JET)

            transrgb = cv2.cvtColor(rgb, cv2.COLOR_RGB2BGR)
            transgt_rgb = cv2.cvtColor(gt_rgb, cv2.COLOR_RGB2BGR)
            

            alpha = 0.5
            rgb_overlay = cv2.addWeighted(np.float32(transrgb * 255), 1-alpha, np.float32(colored_heatmap), alpha, 0)
            gt_rgb_overlay = cv2.addWeighted(np.float32(transgt_rgb * 255), 1-alpha, np.float32(colored_heatmap), alpha, 0)
            
            if not os.path.exists(os.path.join(cfg.basedir, cfg.expname, f'render_test_heatmap')):
                os.makedirs(os.path.join(cfg.basedir, cfg.expname, f'render_test_heatmap'))
            
            cv2.imwrite(os.path.join(cfg.basedir, cfg.expname, f'render_test_heatmap/{i}.png'), transrgb * 255)
            cv2.imwrite(os.path.join(cfg.basedir, cfg.expname, f'render_test_heatmap/gt_{i}.png'), transgt_rgb * 255)
            cv2.imwrite(os.path.join(cfg.basedir, cfg.expname, f'render_test_heatmap/{i}_heat.png'), rgb_overlay)
            cv2.imwrite(os.path.join(cfg.basedir, cfg.expname, f'render_test_heatmap/gt_{i}_heat.png'), gt_rgb_overlay)
...
def run_render(args, cfg, data_dict, device, debug=True, add_info=""):
    ...
    if args.render_test or args.render_train or args.render_video:
        ...
        if use_merged:
            ...
            model = utils.load_model(model_class, ckpt_path, cfg).to(device)
        stepsize = cfg.fine_model_and_render.stepsize
        render_viewpoints_kwargs = {
            'model': model,
            'ndc': cfg.data.ndc,
    ...
    if args.render_train:
        ...
        rgbs, depths, bgmaps, train_psnr = render_viewpoints(cfg=cfg,
    ...
    if args.render_test:
        ...
        if data_dict['i_test'][0] >= len(data_dict['images']):  # gt images are not provided
            ...
            gt_imgs = [data_dict['images'][i].cpu().numpy() for i in data_dict['i_test']]
        rgbs, depths, bgmaps, psnr = render_viewpoints(
    ...
    if args.render_video:
        ...
        os.makedirs(testsavedir, exist_ok=True)
        print('All results are dumped into', testsavedir)
        rgbs, depths, bgmaps, video_psnr = render_viewpoints(
                cfg=cfg,
